class TypeCardsController < ApplicationController
  before_action :set_type_card, only: [:show, :edit, :update, :destroy]
  ...
  # POST /type_cards
  # POST /type_cards.json
  def create
    @type_card = TypeCard.new(type_card_params)

    respond_to do |format|
      if @type_card.save
        format.html { redirect_to @type_card, notice: 'Type card was successfully created.' }
  ...
  # PATCH/PUT /type_cards/1
  # PATCH/PUT /type_cards/1.json
  def update
    respond_to do |format|
      if @type_card.update(type_card_params)
        format.html { redirect_to @type_card, notice: 'Type card was successfully updated.' }
  ...
  private
    ...
    def type_card_params
      params.require(:type_card).permit(:name)
    end
end
